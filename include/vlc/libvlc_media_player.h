...
/**
 * Description for video, audio tracks(跟踪) and subtitles（字幕）. It contains
...
/**
 * Rectangle type for video geometry（视频几何矩阵类型）
 */
...
/**
 * Marq options definition（字幕选项定义）
 */
 ...
 *	通过引用计数原理释放lib_media_player_t
 * \param p_mi the Media Player to free
 */
LIBVLC_API void libvlc_media_player_release( libvlc_media_player_t *p_mi );

/**
 * Retain(保留) a reference to a media player object. Use
 ...
 * previous（以前的） md will be released.
 *
...
/**
 * Callback prototype（原形） to allocate(分配) and lock a picture buffer.
 *
 * Whenever a new video frame（帧） needs to be decoded, the lock callback is
 * invoked. Depending on the video chroma(色调), one or three pixel planes of
