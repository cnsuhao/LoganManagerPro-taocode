...
namespace DuiLib
{
	...
	void  CSliderUI::SetFrontImage(LPCTSTR pStrImage)
	{
		m_sFrontImage = pStrImage;
		Invalidate();
	}

	LPCTSTR CSliderUI::GetFrontImage()
	{
		return m_sFrontImage;
	}

	void    CSliderUI::SetSpectrumImage(LPCTSTR pStrImage)
	{
		m_sSpectrumImage = pStrImage;
		Invalidate();
	}

	LPCTSTR CSliderUI::GetSpectrumImage()
	{
		return m_sSpectrumImage;
	}

	void	CSliderUI::SetSpectrumValue(int nValue)
	{
		m_nPeak = nValue;
		if(m_nPeak > m_nMax)
			m_nPeak = m_nMax;
		else if(m_nPeak < m_nMin)
			m_nPeak = m_nMin;
		Invalidate();
	}
	...
	void CSliderUI::PaintStatusImage(HDC hDC)
	{
		...
		//»­Ç°¾°Í¼
		if (m_sFrontImage.IsEmpty() == false)
		{
			RECT rcFront = m_rcItem;
			CDuiString sImageModify;
			if (m_bHorizontal)
			{
				int length = m_rcItem.right - m_rcItem.left;
				int value = m_nValue * length / (m_nMax - m_nMin);
				rcFront.right = rcFront.left + value;
				sImageModify.Format(_T("%s%d,%d%s"),_T("source='0,0,"),value,m_rcItem.bottom-m_rcItem.top,_T("'"));
			}
			else
			{
				int height = m_rcItem.bottom - m_rcItem.top;
				int value = m_nValue * height / (m_nMax - m_nMin);
				rcFront.top = rcFront.bottom - value;
				sImageModify.Format(_T("%s%d,%d%s"),_T("source='0,0,"),m_rcItem.right-m_rcItem.left,value,_T("'"));
			}
			CRenderEngine::DrawImageString(hDC,m_pManager,rcFront,rcFront,m_sFrontImage,sImageModify);
		}
		//»­ÆµÆ×
		if (m_sSpectrumImage.IsEmpty() == false && m_nPeak != -1)
		{
			CDuiRect rcSpectrum = m_rcItem;
			CDuiString sImageModify;
			if (m_bHorizontal)
			{
				int length = m_rcItem.right - m_rcItem.left;
				int value = m_nPeak * length / (m_nMax - m_nMin);
				rcSpectrum.right = rcSpectrum.left + value;
				sImageModify.Format(_T("%s%d,%d%s"),_T("source='0,0,"),value,m_rcItem.bottom-m_rcItem.top,_T("'"));
			}
			else
			{
				int height = m_rcItem.bottom - m_rcItem.top;
				int value = m_nPeak * height / (m_nMax - m_nMin);
				rcSpectrum.top = rcSpectrum.bottom - value;
				sImageModify.Format(_T("%s%d,%d%s"),_T("source='0,0,"),m_rcItem.right-m_rcItem.left,value,_T("'"));
			}
			CRenderEngine::DrawImageString(hDC,m_pManager,rcSpectrum,rcSpectrum,m_sSpectrumImage,sImageModify);
		}
		//»­»¬¿é
		RECT rcThumb = GetThumbRect();
