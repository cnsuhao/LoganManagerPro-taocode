...
void CMenuElementUI::DoPaint(HDC hDC, const RECT& rcPaint)
{
	...
	else if(m_strFrontImage.IsEmpty() == false)
		DrawFrontImage(hDC,m_rcItem);
...
void CMenuElementUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("frontimg")) == 0 )
		m_strFrontImage = pstrValue;
	else
...
void CMenuElementUI::DrawFrontImage(HDC hDC, const RECT& rcItem)
{
	TListInfoUI* pInfo = m_pOwner->GetListInfo();
	RECT rcFront = rcItem;
	rcFront.right = pInfo->rcTextPadding.left;
	rcFront.top += pInfo->rcTextPadding.top;
	rcFront.bottom -= pInfo->rcTextPadding.bottom;
	CRenderEngine::DrawImageString(hDC, m_pManager, rcFront, m_rcPaint, m_strFrontImage, NULL);
}
