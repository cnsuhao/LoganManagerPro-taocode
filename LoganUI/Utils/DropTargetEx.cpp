...
CDropTargetEx::CDropTargetEx(WindowImplBase *pMainWnd):
m_pMainWnd(pMainWnd),
tb_RefCount(0),
m_hTargetWnd(0),
m_AcceptKeyState(0),
...
BOOL CDropTargetEx::DragDropRegister(HWND hWnd,	DWORD AcceptKeyState)
{
	if(!IsWindow(hWnd))
		return false;
	HRESULT s = ::RegisterDragDrop(hWnd,this);//需要OLE组件,在main函数中加入OleInitialize(NULL);
	if(SUCCEEDED(s))
	{ 
		m_hTargetWnd = hWnd;
		m_AcceptKeyState = AcceptKeyState; 
		
		GetWindowRect(m_hTargetWnd,&m_rcList);
		int nWidth = m_rcList.GetWidth();
		int nHeight = m_rcList.GetHeight();
		m_rcList.left = 0;
		m_rcList.top = 0;
		m_rcList.right = nWidth;
		m_rcList.bottom = nHeight;
		return true;
	}
	else 
	{ 
