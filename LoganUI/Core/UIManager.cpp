...
{
    //if( m_pStrResourceZip == _T("membuffer") ) // 考虑换肤所以得注释
		//return;
...
const TImageInfo* CPaintManagerUI::GetImageEx(LPCTSTR bitmap, LPCTSTR type, DWORD mask, bool gray)
{
	TImageInfo* data = NULL;
	if(gray)
	{
		//生成查找灰度图像的key
		CDuiString grayBitmap = bitmap;
		grayBitmap += _T(".gray");
		data = static_cast<TImageInfo*>(m_mImageHash.Find(grayBitmap.GetData()));
		if(!data)
		{
			if(AddImage(bitmap, type, mask, gray))
			{
				data = static_cast<TImageInfo*>(m_mImageHash.Find(grayBitmap.GetData()));
			}
		}
...
const TImageInfo* CPaintManagerUI::AddImage(LPCTSTR bitmap, LPCTSTR type, DWORD mask,bool gray)
{
    TImageInfo* data = NULL;
    if( type != NULL ) {
        if( isdigit(*bitmap) ) {
            ...
            data = CRenderEngine::LoadImage(iIndex, type, mask, gray);
        }
    }
    else {
        data = CRenderEngine::LoadImage(bitmap, NULL, mask,gray);
    }
	...
	if(gray)
	{
		...
		CDuiString grayBitmap = bitmap;
		grayBitmap += _T(".gray");
		if(!m_mImageHash.Insert(grayBitmap.GetData(), data))
		{
